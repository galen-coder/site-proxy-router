...
<p align="center">
  ...
  <img alt="Version" src="https://img.shields.io/badge/version-2.3.0-6C63FF">
</p>
...
如果一个地址已经命中“不走代理列表”，它会直接连接，即使该地址同时配置了临时代理或普通分流规则。
...
- 使用新的简洁分流图标；
- 启用状态改为图标右下角绿色状态点；
- 弹窗更加醒目地区分不走代理、临时路由、命中规则、默认代理和未匹配直连；
- 代理测试仅覆盖测试域名；
- 非 2xx 响应不再被误判为测试成功。
